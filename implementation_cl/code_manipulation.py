...
class Program:
    ...
    def get_decorated_program_str(self, func_to_evolve, func_to_run) -> str:
        ...
        for f in self.functions:
            function_name = re.findall(r'def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(', str(f))[0]
            if function_name == func_to_evolve:
                prog_list.append('@funsearch.evolve')
            elif function_name == func_to_run:
                prog_list.append('@funsearch.run')
            prog_list.append(str(f))
        program += '\n'.join(prog_list)
        return program
